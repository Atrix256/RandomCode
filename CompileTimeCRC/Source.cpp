...
constexpr unsigned int crc32(const char *s) {
    return crc32_rec(0, s);
}

int main(int argc, char **argv)
{
    const char *blah = "Hello";
    unsigned int hash = crc32(blah);  // Debug/Release: Calculated at runtime
    constexpr unsigned int hash2 = crc32("Wassup");  // Debug: Runtime.  Release: Compile Time
    switch (hash) {
        case hash2: {  // Debug / Release: Compile Time
            printf("A");
            int ijkl = 0;
            break;
        }
        case crc32("Hello"): {  // Debug / Release: Compile Time
            printf("B");
            int ijkl = 0;
            break;
        }
...
/*
TODO:
* need to do an exaustive test to see if this crc matches the other one.
* does it match both for win32 and x64?

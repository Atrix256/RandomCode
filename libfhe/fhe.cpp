...
void fhe_initialize(unsigned long int mySecurityParameter) {
    ...
    if ((randomFile = fopen(RANDOM_FILE, "rb")) == NULL) {
...
int main(int argc, char **argv) {
    ...
    */
    ...
    for (int i = 0; i < 16; i++) {
        ...
        mp_int **fa = fhe_encrypt_integer(factor);
        mp_int **fb = fhe_encrypt_integer(factor);
        tmp1 = fhe_multiply_integers(fa, fb);

        result = fhe_decrypt_integer(tmp1);
        DESTROY_ENCRYPTED_INTEGER(tmp1);
